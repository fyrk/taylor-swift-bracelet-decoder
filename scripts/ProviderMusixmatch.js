// from Spicetify Lyrics Plus

export const ProviderMusixmatch = (function () {
  const headers = {
  ...
  async function findLyrics(info) {
    ...
    if (body["matcher.track.get"].message.header.status_code !== 200) {
      return {
        error: `Requested error: ${body["matcher.track.get"]}`,
        uri: info.uri,
      }
  ...
  async function getKaraoke(body) {
    ...
    const parsedKaraoke = JSON.parse(result.richsync.richsync_body).map(
      line => {
        ...
        const text = words.map((word, index, words) => {
          ...
          const time = !isNaN(nextWordStartTime)
